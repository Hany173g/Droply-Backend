...
export async function createComment(userId: string, targetId: string, targetType: string, content: string) {
    ...
    try {
        ...
        const comment = new Comment({ user: new Types.ObjectId(userId), targetId, targetType, content });
...
export async function deleteComment(commentId: string, userId: string) {
    ...
    try {
        ...
        if (comment.user.toString() !== userId) throw ApiError.forbidden("You can only delete your own comments")
        await getTargetModel(comment.targetType).updateOne({ _id: comment.targetId }, { $inc: { commentsCount: -1 } }, { session })
...
export async function getComments(targetId: string, targetType: string, userId?: string, queryString?: Record<string, any>) {
    ...
    if (userId) {
        ...
        isLikes.map(like => comments.map(comment => {
