...
export async function getNotificationsByUser(userId: string, queryString: Record<string, any>) {
    ...
    let [notifications, unreadCount, total] = await Promise.all([
        features.query.populate("fromUserId", "name username photo").populate("targetId", "title thumbnail slug type views").lean(),
...
export async function buildNotificationMessage(fromUserId: string, type: notificationType, likeType?: string): Promise<string | null> {
...
export async function createSingleNotification(userId: string, fromUserId: string, type: notificationType, targetId?: string, likeType?: string) {
