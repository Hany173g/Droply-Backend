...
function checkHeartBeat(timeWatching : number , updatedAt : number) {
    ...
    if (timeWatching > elapsedSeconds) {
        throw ApiError.unprocessableEntity("The heart beat is correct , stop cheating please or report if this problem")
    }
}



export async function watchingTrack(token : string , timeWatching : number , userVideoId : string , ip : string) {
    const session = await mongoose.startSession();
    try {
        await session.startTransaction();
        let watchSession = await WatchSession.findOne({token}).populate<{ userVideoId: IUserVideo }>("userVideoId")
        if (!watchSession) {
...
export async function getWatchHistory(userId: mongoose.Types.ObjectId, queryString?: Record<string, any>) {
    let baseQuery = WatchSession.find({ userId })
        .populate({ path: "userVideoId", select: "title slug thumbnail type userId", populate: { path: "userId", select: "name username" } });
