...
import { createVideoFlow, getUserVideos, getPublicUserVideos, getUserVideo, updateVideoStatus, refreshSignedUrl, searchVideos } from "./video.service.js";
...
export const getPublicVideos = asyncHandler(async (req : Request ,res : Response , next : NextFunction) => {
    let username = req.params.username as string
    let result = await getPublicUserVideos(username)
    res.status(200).json({ success: true, data: result })
})

export const getSingleVideo = asyncHandler(async (req : Request ,res : Response , next : NextFunction) => {
