...
export async function waitForEagerCompletion(
    publicId: string,
    videoId: string,
    backOffDelay: number,
    job: Job,
...
export async function uploadVideoAndThumbnail(
    ...
    await UserVideo.findByIdAndUpdate(userVideo._id, {
        $set: { videoId: video._id, upload: "complete", thumbnail: thumbnailData },
    })
    await fsPromises.unlink(filePath).catch(() => {})
...
export async function mergeWatchTimeToVideo<T extends IUserVideo>(
    videos: T[],
    userId: string,
): Promise<T[]> {
...
export async function createVideoFlow(
    data: IUserVideo,
    userId: string,
