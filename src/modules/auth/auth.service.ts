...
export async function Register(name : string , password : string , email : string , acceptLanguage?: string) {
    ...
    let user = await User.create({name , username , email,password,verificationExpiresAt,language})
...
export async function updatePassword(verifyToken : string , password : string) {
    let checkToken = await VerifyToken.findOne({type:"progress-forget-password",token:verifyToken})
    checkExpiredToken(checkToken)
    if (!checkToken) return  // Skip typescript errors
    password = await hashPassword(password)   
...
export async function sendLoginAlert(name : string , email : string , device : string , location : string) {
    let now = new Date();
...
async function handleSuccessfulLogin(user : IUser & {_id: Types.ObjectId} , ipAddress : string , userAgent : string) {
...
export async function twoFactorAuthenticationFlow(token : string ,verificationId : string , userAgent : string , ipAddress : string) { 
        ...
        const { accessToken, refreshToken } =
            await handleSuccessfulLogin(user, ipAddress, userAgent);
