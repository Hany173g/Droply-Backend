...
async function handleLinks(links : IChannelLink[]  , channel : HydratedDocument<IChannel>)  :  Promise<HydratedDocument<IChannel>>{
    let linksAndLogo = []
    if (links.length > 10) {
    ...
    for (const link of links) {
        ...
        if (!checkUrl) {
            throw ApiError.unprocessableEntity(`This link is not match , example (${link.link}/User) `)
        }
...
export async function updateChannelInfo(data : IChannel,userId : string)  : Promise<IChannel> {
    ...
    if (data.location?.country) {
        ...
        if (!flagUrl) {
            throw ApiError.unprocessableEntity(`Country "${data.location.country}" is not supported`);
        }
        channel.location = {
...
export async function updateBannerOrPhotoChannel(file : Express.Multer.File , type : string , userId : string) {
    let channel = await Channel.findOne({userId:userId})
    if (!channel) throw ApiError.notFound("Channel is delete")
    if (type == "photo") {
...
export async function handleSubscription(userId : string , channelId : string) {
    ...
    try {
        ...
        let isSubscriber = await Subscription.findOne({subscriber : userId , channel:channelId}).session(session)
        if (!isSubscriber) {
        ...
        await Channel.updateOne({_id: channelId }, {
            $inc:{
                subscribers : action
            }
        } , {
...
export function getChannelSubscription(
