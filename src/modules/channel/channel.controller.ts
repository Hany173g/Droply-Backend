...
import {
    updateChannelInfo as updateChannelInfoService,
    getChannel as getChannelService,
    updateBannerOrPhotoChannel,
    handleSubscription as handleSubscriptionService,
    toggleNotification as toggleNotificationService,
...
export const updateBannerPhoto = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        let user = (req as any).user
        let type = req.body.type as string
        let channel = await updateBannerOrPhotoChannel(
...
export const handleSubscription = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        let user = (req as any).user
...
export const toggleNotification = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        let user = (req as any).user
        let channelId = req.params.channelId as string
