...
import {mockVerifyTokenExpired,mockFindVerifyToken,mockFindUser , mockVerifiedUser} from "../helpers/user.service.helper"
...
describe("Auth Service", () => {
    ...
    describe("Email Valadtion", () => {
        ...
        it("should throw if email is duplicate", async () => {
            jest.spyOn(User, "findOne").mockResolvedValue({ id: 1, user: "Droply" } as unknown as UserDocument)
    ...
    describe("Register", () => {
        ...
        it("should hash throw error if is faild", async () => {
            ...
            await expect(authService.Register("Droply", "droplypassword5!", "droply@gmail.com")).rejects.toThrow()
        })
    ...
    describe("verifyAccount", () => {
        it("should throw if verify token not found", async () => {
            ...
            await expect(authService.verifyAccount("randomToken")).rejects.toThrow("Verification Token not found or expired")
        })
        it("should throw if verify token is expired", async () => {
            ...
            await expect(authService.verifyAccount("expiredToken")).rejects.toThrow("Verification Token is expired")
        })
        it("should throw if user not found", async () => {
            let expiredToken = { ...validToken, expiredAt: new Date(Date.now() + 60000) }
            mockVerifyTokenExpired(expiredToken)
            jest.spyOn(User, "findByIdAndUpdate").mockResolvedValue(null as never)
            await expect(authService.verifyAccount("validToken")).rejects.toThrow("User not found or deleted")
        })
        it("should throw if faild create channel" , async() => {
            ...
            await expect(authService.verifyAccount("validToken")).rejects.toThrow("Faild to verify account, please try again")
        })
    ...
    describe("forgetPassword", () => {
        ...
        it("should delete old tokens and create new one", async () => {
            ...
            const deleteMany = jest.spyOn(VerifyToken, "deleteMany").mockResolvedValue({ deletedCount: 1 } as never)
            const create = jest.spyOn(VerifyToken, "create").mockResolvedValue({ _id: "token456" } as never)
    ...
    describe("Send to factor authentication" , () => {
        it("should throw error if faild create verify token" , async() => {
              ...
              jest.spyOn(VerifyToken,"create").mockResolvedValue(null as never)
              await expect(authService.sendtwoFactorAuthentication({_id:343,email:"droply@gmail.com"} as never)).rejects.toThrow()
        })
        it("should sent email if success" , async () => {
            ...
            const sentEmailMock = (await import("../../src/utils/sentEmail")).default as jest.Mock
            sentEmailMock.mockClear()
            await authService.sendtwoFactorAuthentication({_id:343,email:"droply@gmail.com"} as never)
            expect(sentEmailMock).toHaveBeenCalledWith(
                "droply@gmail.com",
